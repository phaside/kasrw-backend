...
def update(id: int, data: TransaksiUpdate):
    ...
    updated = {k: v for k, v in data.dict().items() if v is not None}
    current_data = {**row, **updated}
    
    cursor.execute(
        "UPDATE transaksi SET tanggal = %s, keterangan = %s, jenis = %s, jumlah = %s WHERE id = %s",
        (current_data["tanggal"], current_data["keterangan"], current_data["jenis"], current_data["jumlah"], id)
    )
    db.commit()
    db.close()
